...
- `main.p8`'s `__gfx__` and `__map__` sections are **generated by `tools/make_assets.py`** (Python + Pillow) from the reference images in `images/` — character sprites (Volt 24×32 at spr 0, Torque 24×32 at spr 3, both facing right, color 14 = transparency key) and the 8×8 decor tiles + 16×16 map. Never hand-edit those sections: change the script and re-run it (it is idempotent). `__sfx__`/`__music__` still belong to PICO-8's built-in editors.
